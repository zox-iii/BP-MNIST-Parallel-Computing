...
#define WEIGHTS_FILE "Model-Neural-Network(OpenMP).dat"
#define TRAIN_REPORT_FILE "Training-Report(OpenMP).dat"
#define TEST_REPORT_FILE "Testing-Report(OpenMP).dat"

#define NUM_THREADS 4                          // 定义线程数

const int num_Training = 1000;         // 训练样本总数
...
#pragma omp threadprivate(out1, target, in2, out2, in3, out3)
...
void show_details(){
    cout << "***********************************************************" << endl;
	cout << "*** Training Neural Network for MNIST database (OpenMP) ***" << endl;
    cout << "***********************************************************" << endl << endl;
...
int training_process(){
    ...
    // 开始迭代
    //#pragma omp for firstprivate(out1, target, in2, out2, in3, out3)
...
int main(int argc, char *argv[]){  
    ...
    show_details();                                 // 输出详细信息
    double begin = omp_get_wtime();                 // 开始时间
    init_matrix();                  // 初始化模型矩阵参数
    #pragma omp parallel num_threads(NUM_THREADS) shared(w1, w2, theta2, theta3, delta1, delta2)
    {
        #pragma omp for reduction(+:loss) //schedule(dynamic) 
        for(int sample = 0; sample < num_Training; sample++){
            for(int i = 0; i < n1; i++){ out1[i] = image_data[sample][i]; }     // 获取单个样本图片
            for(int i = 0; i < n3; i++){ target[i] = label_data[sample][i]; }   // 获取单个样本标签
            int nIterations = training_process();//样本训练=正向传播+反向训练
            // 计算损失、输出到report中
            loss += res;      // 计算总的训练损失
            #pragma omp critical
    ...
    double end = omp_get_wtime();                  //结束时间
    double running_time = double(end - begin); //计算运行时间(秒)
    report << endl << "Total running time: " << running_time / 60 << " min" << endl;
    report << "Running time of a sample: " << running_time / num_Training << " s" << endl;
